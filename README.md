...
- 历史影像抽屉提供公开 Landsat 语料工作台：前端按真实 WGS84 窗口、日期和云量检索，后端固定访问 Microsoft Planetary Computer 的 `landsat-c2-l2` collection，只返回具备 Blue/Green/Red/NIR 四个 Raster Extension 标度的候选。可勾选 1–10 景完整覆盖候选并维护逐景资产编号/名称、批次编号和依据；服务端只接收 Item ID，不接收浏览器 URL，重新读取全部 STAC Item、临时申请 SAS、验证完整覆盖和同网格 COG，应用 `scale`/`offset`/`nodata` 后生成浮点地表反射率 GeoTIFF，并且只调用一次统一影像批次服务。任一景失败时全部临时子集和数据库记录一起回滚；成功时原子保存有序实体、批次 manifest SHA-256、WRS、产品、许可和非法定成果说明。SAS 不写入浏览器响应、数据库、日志或实体标签。
...
5. 专题制图已实现持久化版式模板、真实波段产品批量成图、PNG/PDF 实体输出、双 SHA-256 校验、公开数据标识、角色审计，以及覆盖当前全部有效 PNG 专题图的版本化实体图集；标准化交付归档已纳入真实专题图、当前图集、监理报告、影像处理血缘、多源资产目录、逐文件 SHA-256 和归档源变化失效判断。源栅格离线介质封存已实现真实容量预估、当前成果包门禁、ZIP64 独立分卷、顶层规范清单、来源快照、版本替代、逐卷下载和下载前全成员复核。数据共享已实现注册、甲方审批、访问申请、一次性凭证、健康检查、调用审计和撤销。历史影像覆盖矩阵和处理问题时间线已按实际入库时相实现；公开 Landsat 历史语料已支持 1–10 景前端多选、服务端全量裁取和一次事务原子入库。当前开发库真实保存 1986–1989 共五个 Landsat-5 时相；两次两景导入批次分别保存 13,036,858 和 12,888,770 字节实体。五景均已通过产品族专用源级证据校验，并通过三个五景原子处理批次完成 CGCS2000 重投影、哈尔滨市真实边界裁剪和七波段真/假彩色与 NDVI 产品，所有必选步骤及四类快视图均已通过实体 SHA-256 复核。这些是可追溯公开子区语料，不冒充已经具备完整 1980–2024 省域历史库；跨轨拼接和完整年度覆盖仍待持续建设。
