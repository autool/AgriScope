...
当前开发进度、验证证据和明确剩余项见 [项目进度](docs/PROJECT_PROGRESS.md)。
...
![AgriScope 项目级自定义地块属性配置](docs/images/custom-plot-attributes.png)

### 自定义属性解译录入

![AgriScope 自定义属性解译录入](docs/images/custom-plot-editing.png)
...
### 公开 Landsat 历史语料检索与实体入库

![AgriScope 公开 Landsat 历史语料检索与实体入库](docs/images/imagery-public-landsat.png)

### 公开 Landsat 多景原子批次入库

![AgriScope 公开 Landsat 多景原子批次入库](docs/images/imagery-public-landsat-batch.png)
...
![AgriScope 多景 L2A/L2 源级原子承认](docs/images/imagery-source-acceptance-batch.png)

### 多景同步骤原子处理

![AgriScope 多景同步骤原子处理](docs/images/imagery-processing-batch.png)
...
![AgriScope 变化检测多算法配置](docs/images/change-detection-multi-algorithm.png)
...
![AgriScope 任务专题图集编排](docs/images/thematic-map-atlas-workflow.png)
...
![AgriScope 源栅格离线介质封存](docs/images/offline-raster-archive.png)
...
![AgriScope 无人机移动疑点采集](docs/images/uav-mobile-capture.png)

截图使用当前开发数据库真实状态：尚无已启动无人机任务，因此移动端显示可操作人员和明确空任务门禁，不填充演示航线或伪造现场疑点。
...
- 支持任务级全量质检不可变批次账本：质检与当前检查结果、规则问题在一个事务内提交，保存任务图斑数量和更新时间快照、项目规则版本与阈值、实际质量判定业务影像实体快照及 SHA-256 摘要、活动自定义字段模式 SHA-256、覆盖/通过/失败数量、平均得分、问题数、耗时、逐规则汇总及稳定操作人角色。前端可复核最近历史，成果包内嵌完整 `quality/quality_runs.json`，不再用瞬时响应或自由文本冒充批次证据。
- 提交内业自检必须锁定并绑定最近一次仍有效且已通过的全量质检批次；服务端重新比对任务更新时间、图斑数、检查数、通过数、平均得分和影像摘要，并在审核记录中保存受外键约束的 `quality_run_code`。图斑、项目规则、活动自定义字段或最新业务影像变化会保留历史账本但立即失效当前质量证据，前端显示具体重检原因并禁用提交；演示影像和更旧业务影像不会误触发重检。
...
- 提供独立 `/field-capture` 移动外业入口，不加载桌面工作台壳层。外业核查员可直接获取 WGS84 GPS 坐标和终端水平精度、填写现场地类/作物与备注、调用相机并上传实体照片；服务端立即执行项目范围校验和 PostGIS 点斑匹配。弱网重复提交按记录载荷和照片 SHA-256 幂等返回，记录已创建但照片中断时保留明确待补传状态；非 HTTPS 页面会提示浏览器无法使用 GPS。
- 提供独立 `/uav-capture` 无人机移动采集入口，只列出已启动、已采集或已处理飞行任务。采集人员一次提交 WGS84 GPS、终端水平精度、带时区时间、疑点分类/级别/说明、可选任务图斑和相机照片；服务端锁定任务、重新执行 Polygon 包含与图斑归属校验，校验 JPEG/PNG/WebP 签名、20MB 上限和像素尺寸，并在同一事务写入照片实体、空间疑点和稳定用户审计。相同采集编号仅在规范化载荷 SHA-256 与照片 SHA-256 同时一致时幂等返回，冲突重试不会覆盖原证据。
...
- 支持 1–10 景 `operational` 业务影像原子执行同一个内置步骤。服务端按资产编号稳定锁定完整流水线，在生成任何成果前复核全部上游实体、大小、SHA-256、任务项目、步骤依赖、算法参数和真实行政区裁剪边界。批量几何处理只支持统一重投影，GCP 和 RPC/DEM 保留单景流程。全部 GeoTIFF 成功后才写入逐景步骤、批次编号和审核事件并提交一次事务。任一景、参数或数据库提交失败时删除本批全部新产物并回滚状态；已有有效步骤只能通过单景流程显式重跑，避免批量误覆盖。
...
- 历史影像抽屉提供公开 Landsat 语料工作台：前端按真实 WGS84 窗口、日期和云量检索，后端固定访问 Microsoft Planetary Computer 的 `landsat-c2-l2` collection，只返回具备 Blue/Green/Red/NIR 四个 Raster Extension 标度的候选。既可选择一景完整覆盖，也可选择 2–10 景跨轨候选联合覆盖；服务端只接收 Item ID，不接收浏览器 URL，重新读取全部 STAC Item，并使用 PostGIS 对真实 Polygon/MultiPolygon 足迹执行逐景与联合覆盖校核。覆盖完整后才临时申请 SAS，每景只裁取与共同目标范围的交集，应用 `scale`/`offset`/`nodata` 生成浮点地表反射率 GeoTIFF，并且只调用一次统一影像批次服务。任一景失败时全部临时子集和数据库记录一起回滚；成功时原子保存查询/裁取范围、逐景/联合覆盖率、景数、批次 manifest SHA-256、WRS、产品、许可和非法定成果说明。SAS 不写入浏览器、数据库、日志或文件标签。
- 源级承认按产品族独立执行。Sentinel-2 继续要求 MSI L2A、已应用 STAC 标度和浮点 BOA 反射率；Landsat 仅接受平台受控导入的 Collection 2 Level-2 实体，并重新校验当前 SHA-256、Planetary Computer/USGS 血缘、产品与 STAC Item 一致性、WRS、四个有序浮点 Blue/Green/Red/NIR 波段、完整标度清单、无签名 Azure COG 地址和公开许可。既支持单步骤承认，也支持 1–10 景一次原子承认辐射定标与大气校正：全部实体先预检，再生成统一承认编号、逐步骤事件和批次事件，只提交一次；任一景失败时整批不改变。承认复用同一实体并记录 `algorithm_executed=false`，但几何校正、裁剪、增强和波段产品仍需生成各自实体。
...
- 支持从已校验 `band_products` 实体栅格批量生成真彩色、标准假彩色和 NDVI 专题图，输出 PNG/PDF 实体文件。版式包含图名、图廓、指北针、比例尺、图例、制图单位、日期和图号；清单同时保存来源 URI/SHA-256、STAC/许可/密级血缘、渲染参数、成图大小与 SHA-256，预览和下载前重新校验实体，公开数据显式标注“非法定调查成果”。任务图集必须覆盖当前全部有效 PNG 专题图，可人工调整页序，由服务端生成封面、分页目录和统一图幅 PDF；图集 ZIP 同时保存 PDF、原始 PNG、manifest 和逐成员 SHA-256，新增或损坏专题图会使旧图集转为历史或失效。
- 成果 ZIP 采用标准目录归档任务矢量、统计、灾害、地块属性导入源工作簿及批次台账、外业记录、外业照片/语音/调查表实体、外业原始 XLSX、证据事件、质量、审核、报告、真实专题图、当前专题图集和独立监理报告；影像源与处理产物通过重新校验的 URI、大小和 SHA-256 血缘引用，多源数据目录同时保存外部来源与已复核实体的受控 URI、大小、媒体类型、SHA-256、核验人和依据。除清单自身外，每个内嵌文件保存大小和 SHA-256；新增专题图、图集、监理报告、数据资产或影像处理产物会使旧包失效，新版本生成时旧版本明确转为审计历史。
- 成果交付页提供独立“离线介质”工作台：服务端重新校验当前成果包、全部 `operational` 业务源影像、真正产生物理输出的已完成处理步骤和任务/项目级已验证多源数据实体，按可配置容量写入可独立解压的 ZIP64 分卷。单个源文件完整落入一个卷，超过卷容量时明确拒绝，不截断；每卷保存卷内清单、成员大小和 SHA-256，顶层规范清单保存卷序、来源类别、成果包快照和全量来源摘要。新版本替代旧版本，成果包或任一来源实体变化后旧封存禁止下载；项目负责人生成，项目负责人和甲方逐卷下载，下载前重新复核当前来源、顶层清单、分卷及全部成员。
...
2. 多时相变化检测已实现真实影像资格、实体配准成果强制绑定、任务与规则快照、已配准目标栅格公共网格卷帘/闪烁/并排、外部候选 GeoJSON 导入、服务端注册的 RGB 平均绝对差分与 RGB 变化向量幅值、算法独立默认阈值、六类变化判读和不可变审计。
...
5. 专题制图已实现持久化版式模板、真实波段产品批量成图、PNG/PDF 实体输出、双 SHA-256 校验、公开数据标识、角色审计，以及覆盖当前全部有效 PNG 专题图的版本化实体图集；标准化交付归档已纳入真实专题图、当前图集、监理报告、影像处理血缘、多源资产目录、逐文件 SHA-256 和归档源变化失效判断。源栅格离线介质封存已实现真实容量预估、当前成果包门禁、ZIP64 独立分卷、顶层规范清单、来源快照、版本替代、逐卷下载和下载前全成员复核。数据共享已实现注册、甲方审批、访问申请、一次性凭证、健康检查、调用审计和撤销。历史影像覆盖矩阵和处理问题时间线已按实际入库时相实现；公开 Landsat 历史语料已支持一景完整覆盖或 2–10 景跨轨真实足迹联合覆盖、逐景交集裁取和一次事务原子入库。当前开发库真实保存 1986–1989 共五个 Landsat-5 时相；两次两景导入批次分别保存 13,036,858 和 12,888,770 字节实体。五景均已通过产品族专用源级证据校验，并通过三个五景原子处理批次完成 CGCS2000 重投影、哈尔滨市真实边界裁剪和七波段真/假彩色与 NDVI 产品，所有必选步骤及四类快视图均已通过实体 SHA-256 复核。这些是可追溯公开子区语料，不冒充已经具备完整 1980–2024 省域历史库；跨轨镶嵌生产、年度缺口补齐和完整省域历史覆盖仍待持续建设。
6. 田间监测站、物联网设备、幂等遥测、设备故障闭环、AI 病虫害模型版本、人工复核与告警送达已实现；病虫害报告已支持真实识别结果组卷、专家会商实体答复、县市省三级审核和 XLSX 电子台账。外业核查移动端 GPS、现场属性、照片实体与弱网补传已实现。无人机任务、地图化规划和移动端 GPS/照片/疑点原子采集已实现；飞控实时接入与 10000 台设备规模验证仍待继续建设。
...
psql "$POSTGRES_DSN" -f scripts/migrations/20260723_thematic_map_atlas.sql
...
psql "$POSTGRES_DSN" -f scripts/migrations/20260723_quality_submission_run_binding.sql
psql "$POSTGRES_DSN" -f scripts/migrations/20260723_quality_imagery_snapshot_invalidation.sql
```
...
psql "$POSTGRES_DSN" -f scripts/migrations/20260723_offline_raster_archives.sql
```
...
公开 Sentinel-2 成图在图面上持续显示“公开数据 · 非法定调查成果”。解译员可在线预览经重新校验的 PNG，项目负责人拥有批量生成和附件下载权限；每次预览与下载分别记录稳定用户编码和角色快照。

任务专题图集固定使用当前全部有效 PNG 专题图作为完整页集合，只允许调整页序，不允许漏选或重复。服务端生成横版封面、分页目录和统一图幅 PDF，并把 PDF、全部原始 PNG 与规范化 `manifest.json` 原子写入 ZIP；数据库保存图集版本、成员顺序、专题图数量/最近时间快照、来源摘要、PDF 页数和 ZIP/PDF/成员 SHA-256。新版本会把旧完成图集转为 `superseded`，专题图新增、损坏或实体证据变化后旧图集禁止下载。当前实现不承担区域网平差、全色融合、高级镶嵌缝线，也不冒充尚未接入的长期历史影像语料。
...
多源目录覆盖影像、矢量、表格、DEM、控制资料、气象、管理信息、无人机和物联网九类数据，保存来源名称/地址、版本、SHA256、CRS、WGS84 范围、时段、密级、业务/演示标识及父资产血缘。页面明确分为“上传实体并登记”和“仅登记外部来源”：外部登记保留调用方校验值并进入待核验；实体上传由服务端流式落盘、检查扩展名/签名/MIME/ZIP 安全结构、计算大小与 SHA-256，事务成功后才发布到 `backend/storage/datasets` 并标记已核验。
...
`review/`、`reports/`、`thematic_maps/`、`thematic_atlases/`、`supervision/` 和 `archive/` 标准目录。
生成前重新校验当前影像源、全部已完成处理产物、专题图、当前图集和监理报告实体；专题图、图集与监理
...
SHA-256。任务图斑、专题图、专题图集、监理报告、多源资产目录或当前影像处理产物发生变化后，
历史包立即禁止作为当前成果下载；生成新版本时旧 `completed` 包统一转为
`superseded`，确保同一任务只有一个当前交付版本。

大体量源栅格不直接复制进在线成果 ZIP，而由独立离线介质模块封存。模块使用
`offline_archives`、`offline_archive_volumes`、`offline_archive_sources` 和
`offline_archive_events` 保存版本、当前成果包快照、来源快照、稳定用户角色和下载审计。
生成前重新校验当前成果包、全部业务源影像、真实处理产物和已验证多源数据实体，源级
承认步骤因未产生新文件而不重复占用介质。全部卷先写入临时目录并逐成员复核，通过后才
原子发布到 `backend/storage/offline-archives`；数据库失败会清理整个新版本目录。
每个 ZIP64 卷可独立解压并包含 `volume_manifest.json`，顶层 JSON 清单保存全部卷和来源
SHA-256。单文件超过容量时拒绝生成，禁止截断；新版本生成后旧版本转为 `superseded`，
成果包或来源快照变化后旧版本禁止下载。该能力证明当前任务实体可执行离线介质封存，
不冒充尚未接入的 1980–2024 长期历史语料或长期保管制度。
...
独立 `/uav-capture` 路由不加载桌面 GIS 壳层，适配手机相机和 GPS。移动端只查询当前用户
可操作且状态为 `in_progress`、`captured` 或 `processed` 的任务；计划中任务必须先由桌面端
启动。一次 multipart 请求携带稳定 `capture_code`、带时区采集时间、WGS84 坐标、终端水平
精度、疑点分类/级别/说明、可选任务图斑和 JPEG/PNG/WebP 照片。服务端锁定任务并重新校验
任务 Polygon 和 `task_plots`，执行 20MB 上限、图像签名、扩展名和像素尺寸检查，然后在同一
事务新增 `uav_artifacts`、`uav_findings` 与两条 `uav_events`。规范化业务载荷摘要与照片
SHA-256 同时一致时才允许同编号安全重放；任一内容变化都会明确拒绝，不产生重复或覆盖证据。

### 地块属性 Excel 逐行导入 / 导出

地块解译面板提供独立“Excel 属性”工作流。用户可导出当前图斑、最多 500 个显式编号，
或在任务有效图斑不超过 500 个时导出任务全量。服务端生成的 `.xlsx` 固定包含
`plot_code`、`expected_version`、`owner_village`、`land_class`、`crop_type`、
`planting_mode` 和 `irrigation_condition`，并在其后追加当前活动字段的
`custom:{field_code}` 列。工作簿附带填写说明、地类下拉选项和不可见 `_schema` 页；该页
保存规范化字段定义 JSON、SHA-256 及单选命名区域，数据库当前值和当前版本直接写入工作簿，
不由浏览器拼装。

回写时只接受固定表头的 `.xlsx`，单批最多 500 行、10MB。服务端校验 ZIP 内部路径、
加密标记、解压体积、损坏条目、公式单元格、重复编号、字段定义快照与摘要、任务归属、
软删除状态、地类/作物逻辑和自定义字段类型，并在锁定任务及全部目标图斑后逐行核对
`expected_version`。字段名称、类型、选项、必填规则、顺序或版本变化后，旧工作簿必须重新
导出；任一行不合法或版本过期时整批零写入。全部通过后，只为实际变化图斑递增版本，同时
保存包含权属村和完整自定义 JSON 的 `plot_versions` 快照、清理旧版本派生质检、失效未执行
重做分支并重开质量门禁。

原始工作簿按服务端 SHA-256 原子保存到 `backend/storage/plot-attribute-imports`，导入批次
持久化原始文件名、大小、SHA-256、总行数、变化/未变化数量、操作人稳定编码、角色快照
和人工判读依据。前端展示最近 20 个批次和校验摘要；内业解译员、质检员和项目负责人
分别通过 `export_plot_attributes` / `import_plot_attributes` 能力执行，客户端角色文本不能
替代后端授权。最终成果 ZIP 会重新打开并复核每个原始工作簿，将实体文件与
`attributes/import_manifest.json` 批次台账一并归档；文件大小、SHA-256、行数或批次
计数不一致时拒绝生成成果包。矢量成果以 `custom:{field_code}` 写入 GeoJSON/KML，
Shapefile 使用稳定 `CUST001` 别名，FileGDB 使用 `custom_{field_code}`；同时附带完整
`attributes/custom_attributes.json` 账本，确保停用和历史字段值不会因格式限制丢失。
...
```text
http://localhost:5173/field-capture?project_code=RS-2026&task_code=RS-2026-045
```

除 `localhost` 开发环境外，浏览器 GPS API 要求 HTTPS 安全上下文。移动端会保存表单草稿、
真实定位精度和待补传记录编号；照片文件本身不会伪装为已离线保存，网络恢复后需保留或
重新选择原照片完成受控上传。相同采集编号仅在任务、采集人和规范化载荷 SHA-256 完全一致
时幂等返回；相同照片重复上传返回已有证据，不增加第二份文件或审计事件。
...
影像预处理页的“时序覆盖 → 公开历史语料”通过
`POST /api/v1/public-imagery/search` 检索 Microsoft Planetary Computer 固定
`landsat-c2-l2` collection。前端只提交 WGS84 `bbox`、开始/结束日期和最大云量；
服务端限制单次经纬跨度、十年日期跨度和候选数量，并只返回包含 `blue`、`green`、
`red`、`nir08` 四波段、完整 Raster Extension `scale`/`offset`/`nodata`、真实平台、
载荷、WRS、云量、采集时间和公开 STAC Item 链接的候选。候选列表明确区分“完整覆盖
裁取范围”和“仅相交”，后者不能提交导入。

`POST /api/v1/public-imagery/import` 只接受服务端允许格式的 Item ID、目标 WGS84 范围、
资产编号、名称和稳定操作人，不接受 COG 或 SAS URL。服务端在权限校验后重新 GET 固定
collection 下的 Item，逐波段验证原始 Azure Blob HTTPS 地址，再通过 Planetary Computer
固定签名端点申请短期 SAS。四个 COG 必须同 CRS、同仿射网格和同尺寸，目标窗口必须完整
位于场景内且不超过配置的像元上限；输出为 Blue/Green/Red/NIR `float32` 地表反射率
GeoTIFF，保存无签名源 URL、STAC Item、USGS 产品、WRS、Collection 2 L2、公开许可、
非涉密分类和“非法定调查成果”说明。临时 SAS 仅存在于一次服务端读取过程，不持久化。

`POST /api/v1/public-imagery/import-batch` 接受 1–10 个唯一 Item ID、一个共同 WGS84
裁取范围、唯一资产编号、批次编号、稳定操作人和至少 10 字依据。服务端先完成全部条目的
重新获取、来源校验、临时签名和四波段裁取，再以 Item ID 生成稳定文件名，并且只调用一次
`ImageryAssetService.upload_assets_batch()`。任何条目失败都会删除已生成和失败中的全部临时
子集，不进入影像资产事务；统一批次服务内部的文件发布或数据库失败也会回滚全部实体和
记录。前端保留每景资产编号/名称草稿，明确显示最多十景、权限、加载、空态、错误和整批
成功清单，不循环调用单景接口。

当前开发数据库的真实联调证据为：

- Item：`LT05_L2SP_118028_19861209_02_T1`
- 采集时间：`1986-12-09T01:39:36.799019Z`
- 平台/载荷：Landsat-5 / TM；WRS Path 118 / Row 028；云量 2%
- 裁取范围：`[126.5, 45.7, 126.8, 45.9]`（EPSG:4326）
- 输出：EPSG:32652、30m、799×764、四波段 `float32`
...
为 `landsat-c2-l2-source-acceptance-v1`，审核事件保存稳定项目负责人和人工依据。源级承认
完成时处理进度为 40%，没有复制源文件，也没有运行或声称运行 DOS1、LEDAPS/LaSRC 等
重复算法；随后已通过五景原子处理生成几何重投影、行政区裁剪和波段产品实体，当前必选
流水线为 100%。增强仍为可选待办，该单景子区不描述为完整省域历史档案。

新增真实两景原子批次联调证据为：

- 批次：`IMB-LANDSAT-HISTORY-1988-1989-HRB`，成员数 2
- Item 1：`LT05_L2SP_118028_19881128_02_T1`，6,718,813 bytes，SHA-256 `e9cdffe0b8197a6bb75ba1821dfad8cf55a58155c556258295f5864b53721273`
- Item 2：`LT05_L2SP_118028_19890912_02_T1`，6,318,045 bytes，SHA-256 `dde687293493c978817a7f1673b7f061bf09743603a0ea43ad95306313fe292c`
- 共同范围：`[126.5, 45.7, 126.8, 45.9]`；两景均为 EPSG:32652、30m、799×764、四波段 `float32`
- 批次总大小：13,036,858 bytes；manifest SHA-256：`8b7a7bd12919166be83bce21ffc453528104a45c421ac903e549c1f0811cf2db`
- 数据库保存一个批次、两个有序成员和一条稳定项目负责人审核事件；实体、标签、响应和数据库均不含 SAS 参数
- 真实失败探针在第一景裁取成功后让第二个 Item 返回 404，最终批次记录、资产记录、发布目录和临时目录均为 0 残留

两景随后已通过下述多景原子源级承认统一满足辐射定标与大气校正门禁；几何校正、行政区
裁剪、增强和波段产品仍按各自实体证据保持待处理，不继承其他时相的下游处理状态。

### 多景源产品级别原子承认

`POST /api/v1/imagery-assets/source-acceptance-batches` 接受 1–10 个唯一 `operational`
L2A/L2 资产、稳定操作人、至少 10 字依据和明确的“未执行算法”确认。每景固定同时处理
`radiometric` 与 `atmospheric` 两个步骤。服务端先读取任务和真实项目成员能力，按资产编号
稳定顺序锁定资产与双步骤，并在改变任何状态前完成全部源文件受控路径、大小、SHA-256、
处理级别、浮点反射率、STAC 标度、产品族、WRS/产品/许可血缘校验。全部通过后生成一个
服务端承认编号，写入每个步骤的实体证据和 `algorithm_executed=false`，追加每步骤审核事件
和一个批次事件，最后只提交一次。任一成员或提交失败时整批回滚，不复制任何影像文件。

真实联调已经执行两个两景批次：

- `SLA-2F08AA37D7194D01852299DD9585F805`：由项目负责人承认 1988-11-28 与 1989-09-12 两景，共 4 个步骤
- `SLA-086B9C4C1F7846DCAE001782B63C7B01`：由内业解译员通过前端承认 1988-05-20 与 1989-05-07 两景，共 4 个步骤
- 每景两个步骤均复用各自 `storage://imagery/assets/...` 源 URI、原大小和原 SHA-256；处理器版本均为 `landsat-c2-l2-source-acceptance-v1`
- 两批各保存 4 条 `source_level_accepted` 和 1 条 `source_level_batch_accepted` 审核事件，角色快照来自真实项目成员
- 失败探针让第二个资产不存在，接口返回 404；第一景仍为 pending、无 `artifact_evidence`，审核记录数量为 0
- 五个真实 Landsat-5 时相未因源级承认产生 `processed` 文件；后续几何校正、行政区裁剪和波段产品均通过独立原子处理批次生成实体

### 多景同步骤原子处理

`POST /api/v1/imagery-assets/processing-batches/execute` 接受 1–10 个唯一
`operational` 影像、一个统一内置步骤、逐景明确参数、稳定操作人和至少 10 字依据。
批次只处理尚无有效产物的步骤，已完成步骤必须回到单景流程显式重跑。服务端按资产编号
稳定锁定资产与全部流水线，先复核任务项目、上游必选步骤、实体路径、格式、大小、
SHA-256、算法参数以及真实行政区裁剪边界证据。批量几何只支持统一重投影，GCP 和 RPC/DEM
保留单景流程；随后生成全部独立 GeoTIFF。只有每景输出均成功
后，才写入批次编号、实际算法参数、输出结构、源/输出 SHA-256、逐景审核事件和一个批次
事件，并只提交一次数据库事务。任一处理、持久化或提交异常会回滚全部步骤并删除本批
新产物；Rasterio 临时 `.aux.xml`、`.msk`、`.ovr` 侧车也会清理。

真实联调先执行了第二个资产不存在的失败探针：接口返回 404，审核记录保持 79 条、
`processed` GeoTIFF 数量保持 16 个，首景几何步骤仍为 pending。随后五景依次完成：

- `IPB-242311D8B3F544A09630C2BA4CB334FB`：五景重投影至 `EPSG:4490`，每景输出 935×627、四波段浮点 GeoTIFF
- `IPB-2BE91356FF744B05A33E15A1728D82AD`：使用数据库哈尔滨市 `230100` 真实行政区边界裁剪五景
- `IPB-E22DC2E7B44D4F58899668C01D0FC0A1`：按 Blue/Green/Red/NIR 四个独立波段生成七波段真彩色、标准假彩色和 NDVI 产品
- 三批各保存 5 条逐景事件和 1 条批次事件，审核记录从 79 增至 97；五景必选流水线均为 100%
- 五景源影像、真彩色、假彩色和 NDVI 快视图均可用，并分别保存 PNG SHA-256；处理目录没有临时侧车残留
- 第二轮失败探针令第二景增强百分位参数越界，接口返回 400；第一景仍未开始生成输出，审核记录保持 97、GeoTIFF 数量保持 31，两景增强步骤均为 pending
- 浏览器弹窗会读取全部真实流水线并自动选择当前存在可执行候选的步骤，展示阻断原因、最多十景选择、真实算法参数、依据校验和整批成功证据
...
| GET | `/api/v1/workbench/tasks/{task_code}/quality-checks/runs` | 查询最近的不可变全量质检快照及当前提交资格、最新批次和过期阻断原因 |
...
| GET | `/api/v1/plot-attribute-fields` | 查询项目活动/停用字段定义及当前模式 SHA-256 |
| POST | `/api/v1/plot-attribute-fields` | 创建项目级自定义字段并重开项目质量门禁 |
| PATCH | `/api/v1/plot-attribute-fields/{field_code}` | 版本化修改字段语义、选项、顺序和启停状态 |
| POST | `/api/v1/plot-attribute-workbooks/tasks/{task_code}/export.xlsx` | 导出任务全部或最多 500 个显式图斑的当前属性和并发版本 XLSX |
| POST | `/api/v1/plot-attribute-workbooks/tasks/{task_code}/import` | 安全解析、受控保存并原子回写逐行不同的地块属性 |
| GET | `/api/v1/plot-attribute-workbooks/tasks/{task_code}/imports` | 查询最近 20 个属性工作簿实体导入批次 |
| POST | `/api/v1/workbench/tasks/{task_code}/submit` | 通过全量质量门禁后提交内业自检 |
| GET | `/api/v1/field-verifications` | 获取内外业核查记录和空间匹配统计 |
| POST | `/api/v1/field-verifications` | 移动端直接创建带 GPS 精度的外业记录并执行 PostGIS 点斑匹配 |
...
| POST | `/api/v1/imagery-assets/source-acceptance-batches` | 预检 1–10 景 L2A/L2 实体后，在一个事务中原子承认每景定标与大气校正双步骤 |
| POST | `/api/v1/imagery-assets/processing-batches/execute` | 预检 1–10 景业务影像完整流水线后，原子生成同一步骤实体产物并单事务保存逐景和批次审计 |
...
| POST | `/api/v1/public-imagery/search` | 从固定 Planetary Computer Landsat collection 检索真实候选，不返回 SAS |
| POST | `/api/v1/public-imagery/import` | 服务端重取并裁取单个公开 Landsat Item 后进入统一影像入库门禁 |
| POST | `/api/v1/public-imagery/import-batch` | 重取并裁取 1–10 景公开 Landsat，全部成功后通过一次影像批次事务原子入库 |
...
| GET | `/api/v1/offline-archives` | 重新校验真实来源并查询容量预估、生成门禁和离线封存版本历史 |
| POST | `/api/v1/offline-archives/generate` | 项目负责人按配置容量原子生成可独立解压的 ZIP64 源栅格分卷 |
| GET | `/api/v1/offline-archives/{archive_code}/manifest` | 经当前来源和实体复核后下载顶层规范清单 |
| GET | `/api/v1/offline-archives/{archive_code}/volumes/{sequence}/download` | 逐成员 SHA-256 复核后下载指定 ZIP64 分卷 |
...
| POST | `/api/v1/change-detection/runs/{run_code}/discover-candidates` | 从服务端注册表选择真实 RGB 变化算法，基于双时相公共网格生成带校验值的未分类候选成果 |
...
| POST | `/api/v1/thematic-maps/atlases/generate` | 按人工页序把当前全部有效 PNG 专题图编排为带封面、目录和成员清单的实体图集 |
| GET | `/api/v1/thematic-maps/atlases/{atlas_code}/download` | 复核图集当前性、ZIP、PDF、manifest 和全部原始成员 SHA-256 后下载 |
...
| GET | `/api/v1/uav/mobile-captures/overview` | 按稳定操作人能力查询已启动且可移动采集的轻量任务列表 |
...
| POST | `/api/v1/uav/missions/{mission_code}/mobile-captures` | 原子提交移动 GPS、照片实体与任务范围内空间疑点，支持双 SHA-256 幂等重试 |
