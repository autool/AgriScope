...
- 支持从当前任务导出带 `expected_version` 的真实地块属性 XLSX，并按图斑逐行填写不同权属村、地类、作物、种植模式和灌排条件；服务端严格检查工作簿结构、任务范围和并发版本后整批原子回写，保存原始文件大小、SHA-256、稳定用户角色和不可变版本。
...
`planting_mode` 和 `irrigation_condition`，并附带填写说明和地类下拉选项；数据库当前值
和当前版本直接写入工作簿，不由浏览器拼装。

回写时只接受固定表头的 `.xlsx`，单批最多 500 行、10MB。服务端校验 ZIP 内部路径、
加密标记、解压体积、损坏条目、公式单元格、重复编号、任务归属、软删除状态、地类与
作物逻辑，并在锁定任务及全部目标图斑后逐行核对 `expected_version`。任一行不合法或
版本过期时整批零写入；全部通过后，只为实际变化图斑递增版本，同时保存包含权属村的
完整 `plot_versions` 快照、清理旧版本派生质检、失效未执行重做分支并重开质量门禁。
