...
5. 专题制图已实现持久化版式模板、真实波段产品批量成图、PNG/PDF 实体输出、双 SHA-256 校验、公开数据标识、角色审计，以及覆盖当前全部有效 PNG 专题图的版本化实体图集；标准化交付归档已纳入真实专题图、当前图集、监理报告、影像处理血缘、多源资产目录、逐文件 SHA-256 和归档源变化失效判断。源栅格离线介质封存已实现真实容量预估、当前成果包门禁、ZIP64 独立分卷、顶层规范清单、来源快照、版本替代、逐卷下载和下载前全成员复核。数据共享已实现注册、甲方审批、访问申请、一次性凭证、健康检查、调用审计和撤销。历史影像覆盖矩阵和处理问题时间线已按实际入库时相实现；公开 Landsat 历史语料已支持 1–10 景前端多选、服务端全量裁取和一次事务原子入库。当前开发库真实保存 1986、1988、1989 三个 Landsat-5 时相，其中批次 `IMB-LANDSAT-HISTORY-1988-1989-HRB` 一次写入两景、13,036,858 字节，manifest SHA-256 为 `8b7a7bd12919166be83bce21ffc453528104a45c421ac903e549c1f0811cf2db`。这些是可追溯公开子区语料，不冒充已经具备完整 1980–2024 省域历史库；跨轨拼接和完整年度覆盖仍待持续建设。
...
两景当前仅完成真实实体入库，辐射定标、大气校正、几何校正、行政区裁剪和波段产品仍按
各自实体证据保持待处理，不继承 1986 时相的源级承认状态。
