...
- The implemented source-level acceptance contract keeps Sentinel-2 L2A and Landsat Collection 2 Level-2 as separate evidence profiles. Landsat acceptance revalidates the current physical checksum, Planetary Computer/USGS lineage, matching STAC/product identity, WRS, public licence, unsigned Azure COG URLs, four ordered floating reflectance bands, and the complete calibration ledger before reusing the same source for radiometric and atmospheric gates. The multi-scene source endpoint accepts 1–10 unique assets, prevalidates every source before mutation, writes one server-generated acceptance code into every step, commits once, and rolls back all rows on any member failure. The built-in multi-scene endpoint separately locks complete pipelines, prevalidates required physical predecessors and per-scene parameters, generates all independent GeoTIFFs before row mutation, commits once, and deletes all new outputs plus temporary sidecars on failure. Five real 1986–1989 Landsat-5 entities now have checksum-verified CGCS2000 reprojection, Harbin administrative clipping, seven-band true/false-color and NDVI products, 100% required-pipeline completion, and physical quicklooks without claiming optional enhancement or complete provincial historical coverage.
