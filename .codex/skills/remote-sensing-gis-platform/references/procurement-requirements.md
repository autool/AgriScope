...
- The implemented public historical-imagery contract searches the server-fixed Microsoft Planetary Computer `landsat-c2-l2` collection by bounded WGS84 bbox, date range, and cloud threshold. Import accepts only a stable Item ID and refetches it server-side, validates unsigned Azure COG hosts, keeps short-lived SAS values in memory only, requires complete target coverage and co-gridded Blue/Green/Red/NIR Raster Extension metadata, applies scale/offset/nodata to a floating surface-reflectance GeoTIFF, and reuses the atomic imagery-ingestion workflow. It persists STAC, product, WRS, provider, licence, public classification, physical size and SHA-256 evidence plus a non-statutory notice. The current real integration includes Landsat-5 Item `LT05_L2SP_118028_19861209_02_T1`, but this single subset is not a complete 1980–2024 provincial corpus.
