...
const partialCandidateCountComputed = computed<number>(() => (
  resultRef.value?.items.filter(item => !item.fully_covers_query).length || 0
))
...
<template>
  <div class="public-archive-panel">
    <a-alert
      ...
      description="检索端点、collection 和四个反射率波段由服务端固定。既可选择单景完整覆盖，也可选择 2–10 景跨轨候选联合覆盖；提交时服务端使用 PostGIS 对真实 STAC Polygon 足迹进行权威无缝覆盖校核，再逐景裁取与目标范围的交集。SAS 不进入浏览器、数据库或文件标签。"
    />
    ...
    <section v-if="resultRef" class="search-result">
      <header>
        <span>
          ...
          <strong>{{ resultRef.total }} 景候选 · {{ completeCandidateCountComputed }} 景完整 · {{ partialCandidateCountComputed }} 景可联合</strong>
        </span>
      ...
      <div v-else class="candidate-list">
        <article
          ...
          @click="toggleCandidate(candidate.item_id)"
        >
          <header>
            <span>
              <input
            ...
            <a-tag :color="candidate.fully_covers_query ? 'green' : 'orange'">
              {{ candidate.fully_covers_query ? '单景完整覆盖' : `足迹覆盖 ${(candidate.query_coverage_ratio * 100).toFixed(1)}%` }}
            </a-tag>
          </header>
...
.candidate-list article.partial { border-style: dashed; }
